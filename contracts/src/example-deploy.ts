...
const keys = privateKeys['version-1'];
...
if (!isDeployed) {
  ...
  const resp = await transaction.send();
  console.log('tx resp.data', transaction.toPretty());
  console.log('tx resp.data', (resp as any).data);
  //   console.log('tx resp.data', resp.data as any);
}
